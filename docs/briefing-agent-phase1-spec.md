# Briefing Agent – Phase-1-Spezifikation

| Feld | Wert |
|------|------|
| **Projekt** | Briefing Agent |
| **Version** | 1.0 |
| **Datum** | 2026-05-12 |
| **Status** | Designphase abgeschlossen, bereit zur Implementierung |
| **Autor** | Sebastian Olbert |
| **Kontext** | Intern, offiziell sanktioniert |
...
- Offiziell sanktioniert intern, IT-Sicherheitsfreigabe vor produktivem Go-Live erforderlich
- Hybrid-Hosting: KI-Layer on-premises (Whisper + LLM auf vorhandener GPU-Infrastruktur), drumherum interne Cloud-Services für Auth, Logging, Backup
- Phase 1 ausschließlich mit fiktiven Test-Daten betrieben (lokales Auth, keine echten Sales-Inhalte)
- Datenhoheit verbleibt im internen Rechenzentrum
...
Briefing Agent ist ein **Personal Briefing Tool**, kein Distribution-System.
...
|---------|-----|------------|
| Audio-Verarbeitung (Whisper) | On-prem GPU intern | DSGVO, Datenhoheit |
| LLM-Inferenz | On-prem GPU intern | DSGVO, Datenhoheit |
| Persistenz (PostgreSQL) | Internes RZ | DSGVO |
| Backend-Anwendung (Spring Boot) | Internes RZ | Standardpfad |
| Frontend-Hosting (Angular) | Internes RZ | Standardpfad |
| Auth-Layer | Phase 1 lokal, Phase 2 Entra ID Cloud | Stufenweise |
| Backup, Monitoring, Logging | Interne Cloud-Services | Bestehende Konventionen |
...
- TDD-Konvention "Ändere NICHT die Tests" gilt analog zu etablierten internen Projekten
...
| **Thema** | Sachgebiet mit zugeordneten Betreuer-Personen ("Produkt Alpha") | verpflichtend |
...
| Einstieg | Aktion |
|----------|--------|
| Globale Suche | "Anna" oder "Vorstand" oder "Produkt Alpha" oder Volltext-Stichwort tippen → Audience-Detail |
...
- API-Keys werden nicht im Klartext in der DB gespeichert, sondern als Secret-Storage-Referenz (Vault, Azure Key Vault, Spring Cloud Vault – nach internem Standard)
...
Da Briefing Agent ein **Personal Briefing Tool** ist, sind Prompt-Templates pro Autor – Änderungen eines Autors beeinflussen nur seine eigenen Summaries.
...
- OAuth2/OIDC-Flow gegen den internen Entra-Tenant
...
- Externe Personen (nicht im internen Tenant) bleiben weiterhin manuell anlegbar (`person.source='manual'`)
...
Vor produktivem Go-Live (Phase 2) mit dem betrieblichen Datenschutzbeauftragten zu klären:
...
- **Angular 18** (konsistent mit etablierten internen Konventionen)
...
- **Internes Corporate Design** wird angewandt: Primärfarbe `#006ec7`, Fira Sans, FontAwesome – CSS-Variablen unter neutralen Namen (`--color-brand-primary`, nicht firmen-spezifische Bezeichner)
...
Mit dem betrieblichen Datenschutzbeauftragten vor Phase-2-Go-Live abzustimmen:
...
- Audience-Seed legt 3 Personengruppen ("Vorstand-Fiktiv", "Mein Team-Fiktiv", "Alle Mitarbeiter-Fiktiv") und 3 Themen ("Produkt Alpha-Test", "Cybersecurity-Test", "Strategie-Test") mit Persona-Beschreibungen an
- Geschäftsdaten in fiktiven Aufnahmen folgen der internen Testdaten-Konvention: BBNR 12345678, VSNR A123456789

### 13.2 Reale Test-Aufnahmen

- Autor (und ggf. Pilot-Kollegen) sprechen fiktive Szenarien selbst ein oder pasten fiktive Texte
...
| 21 | Sprach-Coverage | deutsch primär, englisch sekundär (b) | branchenbezogene Begriffe + technische Termini |
...
*Ende der Spezifikation. Bei der Implementierung gelten die etablierten internen Konventionen: TDD-orientiertes Vorgehen, "Ändere NICHT die Tests"-Prinzip, Date-stamped Documentation `docs/YYYYMMDD/`, Stop-Hooks für Regressionstests.*
